...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function evade() {
    if (isChaseCaught || chaseAttempts >= MAX_CHASE_ATTEMPTS) return;

    chaseAttempts++;
    const rect = chaseArea.getBoundingClientRect();
    const bw = chaseBtn.offsetWidth || 120;
    const bh = chaseBtn.offsetHeight || 44;
    const pad = 12;
    const maxX = Math.max(pad, rect.width - bw - pad);
    const maxY = Math.max(pad, rect.height - bh - pad);

    const randX = Math.floor(Math.random() * (maxX - pad)) + pad;
    const randY = Math.floor(Math.random() * (maxY - pad)) + pad;

    chaseBtn.style.left = randX + 'px';
    chaseBtn.style.top  = randY + 'px';
    chaseBtn.style.transform = 'none';

    if (chaseAttempts >= MAX_CHASE_ATTEMPTS) {
      chaseBtn.textContent = taunts[taunts.length - 1];
      chaseBtn.classList.add('btn-catchable');
    } else {
      chaseBtn.textContent = taunts[chaseAttempts - 1];
    }
  }

  function catchChaseButton() {
    if (isChaseCaught) return;
    isChaseCaught = true;
    chaseBtn.classList.remove('btn-catchable');
    chaseBtn.textContent = 'You got me! 🥹💖';
    chaseBtn.style.background = 'linear-gradient(135deg, #e8c547, #fdd9b5)';
    chaseBtn.style.color = '#3d2c3e';
    chaseBtn.style.pointerEvents = 'none';
    caughtText.classList.remove('hidden');
    boom();
    setTimeout(() => goTo('slide-rate'), 1300);
  }

  // Desktop hover evasion
  chaseBtn.addEventListener('mouseenter', () => {
    if (chaseAttempts < MAX_CHASE_ATTEMPTS) {
      evade();
    }
  });

  // Click handler (works for mouse, keyboard, and tapped devices)
  ...
  chaseBtn.addEventListener('touchstart', (e) => {
    e.preventDefault();
    if (chaseAttempts < MAX_CHASE_ATTEMPTS) {
      evade();
    } else {
